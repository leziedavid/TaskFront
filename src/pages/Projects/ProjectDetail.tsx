...
import { getFilteredTasks, getTaskByProjectId } from '../../services/TaskService';
...
const ProjectDetail: React.FC = () => {
    ...
    const handleClick = async (publicId: string) => {
        const url = `${process.env.REACT_APP_FILE_BASE_URL}/${publicId}`;
    ...
    const getFilteredTask = async () => {
        try {
            ...
            setTasks(apiResponse.data ?? null); // Utilisez l'opérateur nullish coalescing
        
            } catch (error) {
            console.error('Error fetching filtered projects:', error);
            toast.error('Failed to fetch projects');
            setTasks(null); // Assurez-vous de gérer les erreurs correctement
        }
    };
    ...
    const handleClickBtn = (projectStartDate: string, projectEndDate: string) => {
        ...
        navigate(`/auth/Admin/add/tâches/${id}`);
    };
    ...
    return (
        <>
            ...
            <Breadcrumb pageTilte="Projet" pageElement={response?.projects?.projectName} pageName="Detail" />
                                            ...
                                            <p className="leading-loose text-gray-500 dark:text-gray-300">
                                                ...
                                                <div className="mb-4 flex flex-col gap-2 xl:flex-row xl:items-start">
                                                    ...
                                                    <div className="w-full xl:w-1/2 flex-1">
                                                        ...
                                                        {/* <input value={new Date(response.projects?.projectEndDate).toLocaleDateString()} disabled className=" bg-white text-lg w-full rounded-lg border border-stroke py-2 px-4 text-black "/> */}
                                                    </div>
                                ...
                                <div className="mb-3 flex flex-col gap-y-4 rounded-sm border border-stroke bg-white p-1 shadow-default dark:border-strokedark dark:bg-boxdark sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    {authorisation === 'ADMIN' || authorisation === 'MANAGER' || authorisation === 'GLOBAL_ADMIN' ? (
                                        ...
                                        <div className="flex justify-end gap-4.5">
                                            ...
                                            <button onClick={() => handleClickBtn(response.projects?.projectStartDate, response.projects?.projectEndDate)} className="flex items-center justify-center rounded-lg border bg-[#012340] py-2 px-6 font-medium text-gray hover:bg-opacity-90 cursor-pointer" type="button">
                                                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-2">
