...
const SelectUsers: React.FC<SelectUsersProps> = ({ setUsers, Departments, setTablegenerate, setDataGenerated }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const apiResponse = await getAllUsersByMultipleDepartment(Departments);
  ...
  const filteredUsers = response?.data?.filter(
    user =>
      ...
      !selectedOptions.includes(user.userId.toString())
  ) || [];

  const handleConfirmSelection = () => {
    const selectedUsers = response?.data?.filter(user => selectedOptions.includes(user.userId.toString())) || [];

    setTablegenerate(prevUsers => {
  ...
  return (
    <>
      ...
      <div ref={selectRef} className="relative z-2 bg-white dark:bg-form-input">
                ...
                {selectedOptions.length > 0
                  ? selectedOptions.map(userId => {
                      ...
                      return (
                        <div
                          key={userId}
                          onClick={() => toggleUserSelection(parseInt(userId))}
                          className={`flex items-center ${user ? 'bg-[#dcdcdc] text-black text-[13px] px-3 py-1 rounded-full' : 'rotate-0'}`}
                        >
