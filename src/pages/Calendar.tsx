...
import { getCalendarsData } from '../services/CalendarsServices'; // Assure-toi que le chemin est correct
import { TaskDataCalendar } from '../interfaces/CalendarsData';
import DetailCalendarModal from '../components/Modal/DetailCalendarModal';
import { ActionDTO } from '../interfaces/ModelsTask';
import { getActionsByTaskId } from '../services/TaskActionServices';
                        ...
                        {hasMultipleTasks ? (
                          ...
                          tasksForDay.map(task => (
                            <div key={task.taskId} className="absolute left-2 top-1 flex w-full flex-col rounded-sm border-l-[3px] bg-gray px-3 py-1 text-left" style={{ borderColor: getRandomColor() }}>
                              <span onClick={() => openModalCalendar(task)}  className="event-name text-[12px] font-semibold text-black dark:text-white">
                                {task.taskName}
                              </span>
                            </div>
                          ))
...
const scrollToTop = () => {
  ...
  return (
    <>
      {/* <Breadcrumb pageName="Le Calendrier" pageTilte={'du'} pageElement={"Jour"} /> */}
