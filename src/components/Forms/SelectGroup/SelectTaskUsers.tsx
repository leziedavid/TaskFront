...
import { getProjectUsersById } from '../../../services/ProjectService';

interface SelectTaskUsersProps {
  setUsers: React.Dispatch<React.SetStateAction<string>>;
  idproject:number | undefined; //
}

const SelectTaskUsers: React.FC<SelectTaskUsersProps> = ({ setUsers,idproject }) => {
  ...
  const fetchUsers = async (id: number) => {
    try {
        const apiResponse = await getProjectUsersById(id);
            ...
            <div className="max-h-60 overflow-y-auto">
              {filteredUsers?.length ? (
                filteredUsers.map(user => (
                  <div key={user.userId} onClick={() => toggleUserSelection(user)} className={`cursor-pointer py-2 px-5 hover:bg-gray-100 dark:hover:bg-form-input-dark flex items-center space-x-2 ${
